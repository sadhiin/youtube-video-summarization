...
class VectorStoreManager:
    ...
    @retry(
        tries=config.VECTOR_STORE_RETRIES,
    ...
    def _load_from_disk(self) -> Optional[FAISS]:
        ...
        try:
            ...
            logging.error(
                f"Error loading vector index for video {self.video_id}: {str(e)}"
            )
            return None
    ...
    def _create_new_store(self, transcript: str) -> Optional[FAISS]:
        ...
        if not transcript or len(transcript.strip()) < 10:
            logging.error(
                f"Error: Transcript for video {self.video_id} is too short or empty"
            )
            return None
        ...
        try:
            ...
            logging.info(
                f"Successfully created vector store with {len(documents)} chunks for video {self.video_id}"
            )
            return vector_store
        except Exception as e:
    ...
    def _test_vector_store(self, vector_store: FAISS) -> None:
        ...
        if not test_results:
            logging.warning(
                f"Vector store created for {self.video_id} but test query returned no results"
            )
    ...
    def create_emergency_store(self, transcript: str) -> Optional[FAISS]:
        ...
        try:
            ...
            document = Document(
                ...
                metadata={"source": "emergency_transcript", "video_id": self.video_id},
            )
            ...
            if video_dir.is_dir():
                ...
                if index_path.exists():
                    try:
                        ...
                        logging.error(
                            f"Error loading vector index for video {video_id}: {str(e)}"
                        )
    ...
    def search_similar_videos(query: str, limit: int = 5) -> List[str]:
        ...
        Returns:
            List of video IDs sorted by relevance
        """
        ...
        for video_id, vector_store in _VIDEO_VECTOR_STORES.items():
            try:
                ...
                similar_docs = vector_store.similarity_search_by_vector(
                    query_embedding, k=3  # Get top 3 chunks from each video
                )

                if similar_docs:
                    # Calculate average relevance score (simplified)
                    # You could implement more sophisticated relevance scoring here
                    relevance_score = len(similar_docs) / 3.0  # Normalize by max chunks
                    ...
                    results.append(
                        {
                            ...
                            "sample_text": (
                                similar_docs[0].page_content[:100]
                                if similar_docs
                                else ""
                            ),
                        }
                ...
                logging.error(
                    f"Error searching vector store for video {video_id}: {str(e)}"
                )

        # Sort by relevance and return video IDs
        results.sort(key=lambda x: (x["matching_chunks"], x["relevance"]), reverse=True)

        if config.DEBUG and results:
            # Log top match details
            top_match = results[0]
            logging.info(
                f"Top match: video_id={top_match['video_id']}, "
                f"relevance={top_match['relevance']}, "
                f"matching_chunks={top_match['matching_chunks']}"
            )
            logging.info(f"Sample text: {top_match['sample_text']}")
