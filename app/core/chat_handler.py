...
class ChatSession:
    ...
    def __init__(self, video_id: str, session_id: Optional[str] = None):
        ...
        self.memory = ConversationBufferMemory(
            ...
            input_key="question",
            output_key="answer",
            human_prefix="Human",  # Make these explicit
            ai_prefix="AI"
        )
    ...
    def load_history_from_db(self, db: Session):
        ...
        for entry in history:  # Assuming get_chat_history returns ordered results
            self.memory.chat_memory.add_user_message(entry.message)
            self.memory.chat_memory.add_ai_message(entry.response)
            
        print(f"Loaded {len(self.memory.chat_memory.messages)//2} conversation turns from database")
...
def create_chat_chain(video_id: str, session: ChatSession):
    ...
    # Create conversational chain with explicit memory configuration
    chain = ConversationalRetrievalChain.from_llm(
        llm=llm,
        retriever=retriever,
        memory=session.memory,
        memory_key="chat_history",  # Ensure this matches memory object
        return_source_documents=True,
        combine_docs_chain_kwargs={"prompt": prompt}
    )
...
def get_chat_response(video_id: str, message: str, db: Session, session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    # Print for debugging
    print(f"Processing chat for video {video_id}, session ID: {session_id}")
    ...
    # Get response - note: we're not passing chat_history here as it's handled by memory
    response = chain.invoke({
        "question": message
    })
    ...
    # Return session_id to ensure continuity
    return {
        "answer": response["answer"],
        "sources": sources,
        "session_id": session.session_id,
        "history_length": len(session.memory.chat_memory.messages) // 2  # Number of interactions
    }
