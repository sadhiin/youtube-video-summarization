...
class ChatRequest(BaseModel):
    ...
    video_id: str
    message: str
    session_id: Optional[str] = None  # Added session_id to maintain chat history
...
class ChatResponse(BaseModel):
    ...
    answer: str
    sources: List[Dict[str, Any]] = []
    session_id: str  # Added to return the session_id for future requests
    history_length: Optional[int] = 0  # Optional field to show history length
...
async def chat_with_video(
    ...
    Maintains conversation history across multiple requests using session_id.
    """
    stored_summary = get_stored_summary(db, chat_request.video_id)
    ...
    try:
        # Pass the session_id to maintain chat history
        logging.info(f"Processing chat request for video {chat_request.video_id} with session {chat_request.session_id}")
        
        response = get_chat_response(
            video_id=chat_request.video_id,
            message=chat_request.message,
            db=db,
            session_id=chat_request.session_id  # Pass the session_id
        )
